...
const Settings = () => {
  ...
  const fetchAdmins = async () => {
    ...
    try {
      ...
      const response = await fetch(getAdminPhpApiUrl('admin/get_all_admins.php'), {
        headers: {
          'Authorization': token || ''
        }
      });
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch(getAdminPhpApiUrl('admin/add_admin.php'), {
        method: 'POST',
        headers: {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch(getAdminPhpApiUrl('admin/delete_admin.php'), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token || ''
